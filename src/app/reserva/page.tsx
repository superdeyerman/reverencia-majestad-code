...
export const metadata: Metadata = {
  title: 'Reserva | Reverencia Majestad · Luxury Hair & Spa',
  description:
...
export default async function ReservasPage({
  ...
  const preselectedSlug = typeof serviceParam === 'string' ? serviceParam : null;

  const [servicios, completedCount] = await Promise.all([
    prisma.service.findMany({
      ...
      select: {
        ...
        prisma.booking.count({
          ...
          const featuredCount = servicios.filter((s) => s.isFeatured).length;
          const homeCount = servicios.filter((s) => s.supportsHome).length;
          const hotelCount = servicios.filter((s) => s.supportsHotel).length;
          ...
          {
            icon: Users,
            value: completedCount > 0 ? `${completedCount}+` : 'Santiago',
            label: 'experiencias completadas',
          },
          {
            icon: Star,
            value: `${avgRating}/5`,
            label: reviewsCount > 0 ? `${reviewsCount} reseñas verificadas` : 'experiencia premium',
          },
          {
            icon: MapPin,
            value: 'Santiago',
            label: 'domicilio · hotel · studio',
          },
        ];
  ...
  return (
    <main className="bg-[#f8f4ed]">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-16 px-6 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:px-12 lg:py-20">
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-10 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
              ...
              <Link
                href={`https://wa.me/56963929354?text=${encodeURIComponent(
                  'Hola, quiero reservar una experiencia premium'
                )}`}
                target="_blank"
                ...
                <div className="relative flex flex-col gap-8 px-7 py-10 text-white sm:px-10 sm:py-12">
                  <div>
                    ...
                    <p className="mt-3 font-serif text-3xl leading-tight text-white sm:text-4xl">
                      {servicios.length} experiencias premium disponibles hoy.
                    </p>
                  </div>

                  <div className="grid grid-cols-3 gap-4">
                    <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 backdrop-blur">
                      <p className="font-serif text-3xl text-white">{servicios.length}</p>
                  ...
                  <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/5 px-5 py-4 backdrop-blur">
                    ...
                    <div className="hidden rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-right backdrop-blur sm:block">
                      ...
                      <p className="mt-1 font-serif text-2xl text-white">
                        {servicios[0]?.basePrice
                          ? new Intl.NumberFormat('es-CL', {
                              style: 'currency',
                              currency: 'CLP',
                              maximumFractionDigits: 0,
                            }).format(servicios[0].basePrice)
                          : '$25.000'}
                      </p>
                    </div>
                  </div>
      ...
      <section id="wizard" className="mx-auto max-w-7xl scroll-mt-24 px-6 py-14 lg:px-10">
        <BookingWizard services={servicios} preselectedSlug={preselectedSlug} />
      </section>
      ...
      <section className="border-t border-stone-200 bg-white py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <div className="mt-12 flex flex-col items-start justify-between gap-5 rounded-[2rem] border border-stone-200 bg-[#171311] px-7 py-7 text-white sm:flex-row sm:items-center sm:px-10">
            ...
            <Link
              href={`https://wa.me/56963929354?text=${encodeURIComponent(
                'Hola, necesito una reserva especial'
              )}`}
              target="_blank"
              rel="noopener noreferrer"
